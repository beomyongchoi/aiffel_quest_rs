...
- **시도**: `SentencePiece` 라이브러리를 활용, 한국어와 영어 코퍼스를 합쳐 **공유 어휘(Shared Vocabulary)** 모델 생성 및 적용 (v5).
...
- **시도 1 (v4.1 - 모델 확장)**: 영어 토크나이저 개선(`spaCy`) 및 모델 용량 증설 (`HID_DIM`, `N_LAYERS` ↑).
...
- **시도 2 (v4.2 - RNN 유닛 변경)**: 안정적인 v4 하이퍼파라미터 기준, RNN 유닛을 GRU에서 **LSTM**으로 교체.
...
- **시도 3 (v4.4 - 최종 조합)**: 검증된 최고 요소(LSTM + `spaCy`) 결합.
    # 한영 번역기 모델 개선 과정 분석 보고서
...
- **최종 가설**: 점진적 개선이 아닌, 아키텍처의 근본적 전환(Transformer)만이 성능의 비약적 도약을 만들 수 있음.
- **최종 실험 (v5.x)**: `Mecab` + `spaCy` 전처리 방식은 유지, 모델 엔진을 **트랜스포머**로 교체.
- **결과**: **Test PPL 69.7 달성.** Seq2Seq(562) 대비 **압도적인 성능 향상.**
